...
const FB_CATALOG_ID = env.FB_CATALOG_ID!;
...
export async function fbCreateProduct(product: any) {
	try {
		...
		form.append(
			'image_url',
			Array.isArray(product.metadata) ? product.metadata[0] : '',
		);
...
export async function fbUpdateStock(
	sku: string,
	amount: number,
	quantityLeft: number,
	currency: string,
...
export async function fbCheckStock(sku: string) {
	try {
		...
		const catalogId = env.FB_CATALOG_ID;
